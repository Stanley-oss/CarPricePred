...
class TabularResNet(nn.Module):
    def __init__(self, num_numerical, cat_dims, embedding_dims,
                 hidden_dim=256, num_blocks=3, dropout=0.2):
        super().__init__()

        # 类别嵌入 (Model, Transmission 等，不含 Brand/Fuel)
        self.embeddings = nn.ModuleList([
            nn.Embedding(n, d) for n, d in zip(cat_dims, embedding_dims)
        ])
        ...
        self.resnet_layers = nn.ModuleList([
            ResNetBlock(hidden_dim, hidden_dim, dropout)
...
class CarDataset(data.Dataset):
    ...
    def __len__(self): return len(self.y_core)

    def __getitem__(self, idx):
        return (self.X_num[idx], self.X_cat[idx],
                self.y_core[idx], self.bias_brand[idx], self.bias_fuel[idx], self.raw_price[idx])
...
def get_data_strategy(file_path, batch_size=64):
    ...
    X_num = df.drop(columns=['Price', 'log_price', 'Brand', 'Model', 'Fuel Type', 'Transmission',
                             'Brand_ID', 'Fuel_ID', 'Model_ID', 'Trans_ID',
                             'bias_brand', 'bias_fuel', 'y_core_raw']).values
    ...
    def pack_data(idx_list):
        return (X_num[idx_list], X_cat[idx_list],
    ...
    return (data.DataLoader(train_ds, batch_size, shuffle=True),
            data.DataLoader(test_ds, batch_size, shuffle=False)), \
           X_num.shape[1], cat_dims, emb_dims, scaler_y, le_brand, le_fuel
...
def main():
    ...
    # 获取数据
    (train_dl, test_dl), n_num, cat_dims, emb_dims, scaler_y, le_brand, le_fuel = get_data_strategy('Full_dataset.csv',
                                                                                                    BATCH_SIZE)
    ...
    header_printed = 1
    with torch.no_grad():
        for x_num, x_cat, y_core, bias_b, bias_f, raw_price in test_dl:
            ...
            if header_printed<=10:
                print("-" * 100)
                print(
                    f"{'Type':<10}|'Brand Coeff'|'Fuel Coeff'|'Base Price'|'Final Pred'|'Actual'")
                print("-" * 100)
