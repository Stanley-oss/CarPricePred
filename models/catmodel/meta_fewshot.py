...
class FewShotConfig:
    ...
    # (Brand, Model, Year) 样本数 >= 这个值就认为“不少”，不做 few-shot,每个 (Brand, Model, Year) 的最小样本数阈值，默认是 30。
    # 如果某个品牌+型号+年份的样本数 ≥ 30，说明数据其实不少，我们就认为主模型足够可靠，不再启用 few-shot 调整
    max_support_size: int = (
        200  # 支持集最多使用多少条样本,同一品牌同一车型的历史支持样本最多用多少条，默认 200，防止 KNN 计算量太大；
    )
    k_neighbors: int = 50  # KNN 中最多取多少邻居,KNN 中最多取多少最近邻，默认 50。
    new_car_max_age: float = 3.0  # Age <= 这个阈值认为是“新车”，才会考虑 few-shot 迁移,新车的年龄阈值，默认是 3 年。
    # 只有 车龄 Age ≤ 3 年 的车，才有资格走元学习逻辑；
...
class FewShotKnnMeta:
    ...
    def __init__(self, cfg: FewShotConfig):
        ...
        self._X: np.ndarray | None = None  # 归一化后的特征矩阵
        self._y: np.ndarray | None = None  # 价格
    ...
    def _load_full_dataset(self):
        ...
        for col in ["year", "age", "milage", "engine", "max_power", "seats", "price"]:
            ...
            )  # 对 year, age, milage, engine, max_power, seats, price 这些关键字段，用 pd.to_numeric 转成数值，非法的转成 NaN。
        ...
        # 把品牌、型号分别做了一个 _norm_str 处理：小写 + 去空格；
        # 再把 year 取整，拼成一个字符串：group_id = brand_key || model_key || year_int

        # 统计每个 (brand, model, year) 的样本量
        self._group_counts = df["_group_id"].value_counts().to_dict()
        self._group_ids = df["_group_id"].to_numpy()
        # 用 value_counts() 统计出每个 group_id 的样本数量，存到 self._group_counts；
        # 同时保存每条样本对应的 group_id 数组到 self._group_ids。
        ...
        X_norm = (
            X_raw - col_means
        ) / col_stds  # 对每一列做标准化 (x - mean) / std，保存均值和标准差到 self._feature_means 和 self._feature_stds。
    ...
    def maybe_adapt(self, d: dict[str, Any], base: dict[str, Any]) -> dict[str, Any]:
        # 给定一辆车的原始特征 d，以及主模型的预测结果 base，决定要不要做 few-shot 调整；
        # 如果要，就算出一个新的 (lo, p50, hi)，返回融合后的结果。
        ...
        # ---------- 从输入样本中取出关键信息 ----------
        brand_raw = d.get("brand") or d.get(
            "Brand"
        )  # 从 d 里读出 brand、model、year、age，并且兼容大小写，比如既支持 "brand" 又支持 "Brand"
        ...
        if n_group >= cfg.min_group_size:
            return base
        # 然后，根据品牌、型号和年份，构造当前样本的 group_id：group_id = brand_key || model_key || year_int
        # 再从一开始统计好的 self._group_counts 里查这个 group_id 的样本数 n_group。
        # 如果 n_group ≥ min_group_size（默认 30），说明这款车在历史数据里样本并不少，主模型本身就有足够的经验，也直接返回 base
        ...
        if len(family_idx) > cfg.max_support_size:
            ...
            if np.isfinite(year_val):
                ...
                family_idx = family_idx[
                    order[: cfg.max_support_size]
                ]  # 按 year_diff 从小到大排序，取最近的 max_support_size 条.
            else:
        ...
        # 这些字段同样通过 _to_number 统一转成数值，缺失的用全局均值填补
        # 然后用一开始保存的 mean/std 做标准化，变成一个标准化后的查询向量 x_query_norm。
        ...
        # 样本越少 & 车越新 -> 权重越大
        w_size = max(
            0.0, min(1.0, (cfg.min_group_size - n_group) / float(cfg.min_group_size))
        )  # 当样本数 n_group 越少，w_size 越接近 1；当样本数逼近 min_group_size，w_size 会趋近 0。
        w_age = max(
            0.0, min(1.0, (cfg.new_car_max_age - age_val) / float(cfg.new_car_max_age))
        )  # 车越新，age 越小，w_age 越接近 1；车越接近新车阈值（比如 3 岁），w_age 越接近 0。
        ...
        # 在有了 w 之后，代码做了一步线性插值：当 w 接近 0 时，几乎就是原始的基础模型输出
        # 当 w 接近 1 时，几乎就是家族历史中位数。
        p50_meta = float(
            np.clip(p50_meta, q10_local, q90_local)
        )  # 无论元学习权重多大，中位数都不应该超出同款车历史 10%~90% 的合理价格区间。

        scale = p50_meta / p50_base if p50_base > 0 else 1.0
        lo_meta = lo_base * scale
        hi_meta = hi_base * scale
        # 在得到新的中位数之后，区间上下界不是重新学的，而是通过一个比例因子来整体缩放：
        # 我们保持原来基础模型区间的形状，只是整体向上或向下缩放，使其中心对齐到新的 p50_meta。
