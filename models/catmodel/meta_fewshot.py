...
class FewShotKnnMeta:
    ...
    def __init__(self, cfg: FewShotConfig, feature_weights: Optional[Dict[str, float]] = None):
        self.cfg = cfg
        ...
        # KNN 使用的特征顺序（和 _load_full_dataset 里的 feat_cols 一致）
        ...
        # 特征权重：来自主模型 feature importance 的字典，例如 {"age":1.2, "milage":0.8,...}
        self._feature_weight_dict = feature_weights or {}
...
#后面在 maybe_adapt 里面，我们可以很快知道当前这辆车所属的 (品牌, 型号, 年份) 一共在历史数据里有多少条样本。
        ...
        self._feat_cols = feat_cols  # 保存在实例里
        ...
        # === 构造特征权重向量 ===
        # 如果 meta 没给权重，就用全 1；否则按字典匹配并归一化到“均值=1”
        w_list = []
        for name in feat_cols:
            w_val = float(self._feature_weight_dict.get(name, 1.0))
            w_list.append(max(0.0, w_val))
        w = np.asarray(w_list, dtype=float)
        if not np.any(w > 0):
            w = np.ones_like(w)
        self._feature_weights = w / w.mean()
        self._df = df
    ...
    def maybe_adapt(self, d: Dict[str, Any], base: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 使用特征权重的加权欧式距离：重要特征在距离里占更大比重
        if self._feature_weights is not None:
            diff = diff * self._feature_weights  # 按列缩放
        ...
        q25_local = float(np.quantile(y_knn, 0.25))
        q75_local = float(np.quantile(y_knn, 0.75))
...
#我们不再仅仅相信全局模型，而是看看同款车在历史上大概都卖多少钱，有一个经验区间。
        ...
        width_base = float(max(0.0, hi_base - lo_base))

        # 基于 IQR 的不确定性缩放：希望最终区间宽度 ≈ iqr_to_width_factor * IQR
        cfg = self.cfg
        s_unc = 1.0
        if iqr_local > 0.0 and width_base > 0.0:
            ...
            s_unc = float(
                np.clip(
                    width_target / width_base,
                    cfg.min_uncertainty_scale,
                    cfg.max_uncertainty_scale,
                )
            )
        # 样本越少 & 车越新 -> 权重越大
        ...
        # 基于 IQR 的不确定性缩放：围绕新的中位数重新展开区间
        delta_lo_base = max(0.0, p50_base - lo_base)
        delta_hi_base = max(0.0, hi_base - p50_base)

        delta_lo_new = delta_lo_base * s_unc
        delta_hi_new = delta_hi_base * s_unc

        lo_meta = p50_meta - delta_lo_new
        hi_meta = p50_meta + delta_hi_new
        ...
        out["meta_info"] = {
            ...
            "width_base": float(width_base),
            "uncertainty_scale": float(s_unc),
            "feature_weights": {
                name: float(val)
                for name, val in zip(self._feat_cols, self._feature_weights)
            }
            if self._feature_weights is not None
            else None,
        }
