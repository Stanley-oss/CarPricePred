...
te_info = _meta.get("target_encoding", {})
brand_te     = te_info.get("Brand")
brand_model_te = te_info.get("BrandModel")
...
def predict_price(d):
    ...
    # 非对称CQR 逐级回退
    # 非对称 CQR（注意：训练时 qlo/qhi 是“比例”，不是绝对金额）
    qlo_ratio, qhi_ratio = hierarchical_qhat_asym(key_full)

    # 原始区间（未截断）——先根据比例算出 lo_raw / hi_raw
    lo_raw = max(0.0, p10_m - qlo_ratio * p50_m)
    hi_raw =           p90_m + qhi_ratio * p50_m
