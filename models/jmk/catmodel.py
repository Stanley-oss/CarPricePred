...
CSV_PATH = 'C:\\Users\\Lenovo\\Desktop\\下载\\Full_dataset.csv'
SAVE_DIR = 'C:\\Users\\Lenovo\\PycharmProjects\\ml\\app\\used_car_price_v4'
TARGET_COL = 'Price'
...
N_SPLITS = 5
ALPHA = 0.2
CATBOOST_PARAMS = dict(depth=8, learning_rate=0.035, l2_leaf_reg=6.0, loss_function='Quantile:alpha={alpha}', iterations=5000, random_seed=RANDOM_STATE, border_count=254, verbose=False, thread_count=-1, od_type='Iter', od_wait=300, subsample=0.9, rsm=0.9, bootstrap_type='Bernoulli')
...
def build_feature_df(df_raw):
    ...
    if GEAR_COL in df.columns:
        df[GEAR_COL] = standardize_enum(df[GEAR_COL], {'a': 'Automatic', 'auto': 'Automatic', 'automatic': 'Automatic', 'm': 'Manual', 'man': 'Manual', 'manual': 'Manual'}, default='Unknown')
    if FUEL_COL in df.columns:
        df[FUEL_COL] = standardize_enum(df[FUEL_COL], {'petrol': 'Petrol', 'gasoline': 'Petrol', 'diesel': 'Diesel', 'cng': 'Other', 'lpg': 'Other', 'hybrid': 'Other', 'electric': 'Other', 'other': 'Other'}, default='Other')
    ...
    numeric_candidates = pick_existing_columns(df, [YEAR_COL, AGE_COL, MILE_COL, HP_COL, ENGINE_COL, SEATS_COL, 'car_age', 'log1p_mileage', 'avg_km_per_year', 'hp_x_auto', 'hp_div_avgkm', 'power_per_cc', 'cc_per_seat', 'brand_count', 'model_count', 'brand_model_count'])
...
if __name__ == '__main__':
    ...
    (q_lo_g, q_hi_g), (q_lo_groups, q_hi_groups) = cqr_asymmetric_global_and_group(y_va.values, va_p10_m, va_p90_m, group_keys, min_group_size=120)
    ...
    df_slice = pd.DataFrame({'y': y_va.values, 'p50': va_p50_m, 'lo': cqr_low, 'hi': cqr_high, 'key': group_keys.values})
    ...
    meta = {'columns': list(X_tr.columns), 'categorical_cols': [c for c in cat_cols if c in X_tr.columns], 'categorical_indices': [X_tr.columns.get_loc(c) for c in cat_cols if c in X_tr.columns], 'alpha': ALPHA, 'market_index': {'period_unit': 'year', 'gamma_t': gamma_t, 'M_t_raw': M_t_raw, 'M_t_smooth': M_t_smooth, 'ewma_alpha': 0.25}, 'cqr_after_market': {'type': 'asymmetric', 'q_lo_global': q_lo_g, 'q_hi_global': q_hi_g, 'q_lo_groups': q_lo_groups, 'q_hi_groups': q_hi_groups, 'group_key_def': 'age_bin(car_age)|fuel_type|transmission|period_bin'}, 'external_ts_blend': {'enable': False, 'blend_weight': 0.5, 'file': os.path.join(SAVE_DIR, 'external_market_forecast.json')}, 'target': TARGET_COL, 'date_col': DATE_COL if DATE_COL in df_raw.columns else None, 'split': {'mode': split_mode, 'train_size': int(len(y_tr)), 'valid_size': int(len(y_va))}, 'winsorized_cols': [c for c in [TARGET_COL, MILE_COL, HP_COL, ENGINE_COL] if c in df_raw.columns]}
